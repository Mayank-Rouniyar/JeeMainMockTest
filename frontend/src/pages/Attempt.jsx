  ...
  <div className="fixed top-0 left-0 w-full h-full bg-gradient-to-tr from-[#0f2027] via-[#203a43] to-[#2c5364] text-white flex flex-col">
    <div className="h-[10vh] w-full flex items-center justify-between px-4 border-b border-white/20">
      <div>
        ...
        <p className="text-xl text-green-400">Mayank Rouniyar</p>
      </div>
      <div className="text-right text-xs leading-tight">
        ...
        <p>
          Test Title:{" "}
          <span className="font-semibold">{tests?.title || "Loading..."}</span>
        </p>
        <p>
          Remaining Time:{" "}
          ...
          <div className="space-y-2">
            {tests?.questions?.[currentQ]?.type === "MCQ" ? (
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="number"
                  className="border p-1 w-40 text-black"
                  value={selectedOptions[currentQ] || ""}
                  onChange={(e) => handleOptionSelect(parseInt(e.target.value))}
                />
                <span className="text-gray-300 text-sm">Enter integer answer</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="w-[30%] flex flex-col border-l border-white/20 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {tests?.questions?.map((_, index) => (
            <button
              key={index}
              className={`w-10 h-10 rounded-full text-white ${getButtonColor(index)}`}
